...
#define OUT_FLE_PATH_SUFFIX "statistica.txt"
...
struct BMPHeader {
    ...
    uint32_t reserved;
    uint32_t dataOffset;
} __attribute__((packed)); // remove padding so that it's compatible with the bmp data layout
...
void write_to_output_file() {
    ...
    int out_fd = creat(out_path_buf, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
    if (out_fd < 0) {
...
void convert_bmp_to_grayscale(const char *path) {
    ...
    for (int i = 0; i < f.info.height; ++i) {
        for (int j = 0; j < f.info.width; ++j) {
            uint8_t *pixel = &f.data[(i * f.info.width + j) * 3]; // store in f.data(acting as buffer now) to overwrite image later
            uint8_t gray =
...
size_t build_input_file_path(char *const *argv, const struct dirent *dr, const char *file_path) {// build file path
...
void build_output_file_path(char *const *argv, const struct dirent *dr) {
    ...
    strncat(out_path_buf, OUT_FLE_PATH_SUFFIX,
            MAX_PATH_LEN - strlen(out_path_buf) - 1);
}

void process_regular_child1(const struct dirent *dr, const char *file_path, struct stat *fst, const int *p1fd,
                            const int *p2fd) {
...
void process_regular_child2(char *const *argv, const int *p1fd, const int *p2fd) {// close write end of pipe1
    close(p1fd[1]);
    ...
    execlp("/bin/sh", "/bin/sh", "./s9.sh", argv[3],
           NULL);
}
...
int process_regular_file(char *const *argv, int valid_sentence_cnt, const struct dirent *dr, const char *file_path,
                         struct stat *fst, int pid, const int *p1fd, const int *p2fd) {
    // this returns the number of correct sentences in the file
    if (pid == 0) { // 1st child
        process_regular_child1(dr, file_path, fst, p1fd, p2fd);

    } else {              // parent
      if ((pid = fork()) < 0) { // create 2nd child to exec script and read from pipe(which should be loaded by the 1st child)
...
void process_bmp(const struct dirent *dr, const char *file_path, struct stat *fst, int pid) {
    if (pid == 0) { // 1st child
...
bool is_bmp(const char *file_path, size_t path_len) {
    return file_path[path_len - 4] == '.' &&
                   file_path[path_len - 3] == 'b' &&
                   file_path[path_len - 2] == 'm' &&
                   file_path[path_len - 1] == 'p';
}
...
int processEntry(char *const *argv, int valid_sentence_cnt, const struct dirent *dr, const char *file_path,
    ...
    } else if (S_ISREG((*fst).st_mode)) { // regular (here we have to send contents through pipe, to the script)
        valid_sentence_cnt = process_regular_file(argv, valid_sentence_cnt, dr, file_path, fst, pid, p1fd, p2fd);
    }
...
int main(int argc, char **argv) {
  ...
  for (; dr != NULL; dr = readdir(dirptr)) {
      valid_sentence_cnt = processEntry(argv, valid_sentence_cnt, dr, file_path, &fst, pid);
  }
